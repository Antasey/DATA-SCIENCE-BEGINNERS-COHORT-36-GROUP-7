...
class MLPredictionApp:
    ...
    def render_charts(self):
        """ Renders a 4-panel spacious dashboard using pure Matplotlib """
        if self.canvas is not None:
            self.canvas.get_tk_widget().destroy()
        ...
        if hasattr(self.model, "feature_importances_") and hasattr(self.model, "feature_names_in_"):
            ...
            top_indices = np.argsort(importances)[-8:] # Showing top 8 to reduce clutter
        ...
        if cat_col and self.df is not None:
            ...
            grouped.plot(kind="bar", ax=ax4, color=["#377eb8", "#ff7f00"], width=0.7)
            
            ax4.set_title(f"Attrition by {cat_col}", fontsize=11, fontweight="bold", pad=12)
            ax4.set_xlabel(cat_col, labelpad=8)
            ...
            ax4.set_title("Attribute Distribution Analysis", fontsize=11, fontweight="bold", pad=12)

        ax4.grid(True, linestyle="--", alpha=0.4)

        # Adjust spacing between subplots (wspace = horizontal space, hspace = vertical space)
        fig.subplots_adjust(wspace=0.35, hspace=0.45, left=0.1, right=0.95, top=0.92, bottom=0.12)
