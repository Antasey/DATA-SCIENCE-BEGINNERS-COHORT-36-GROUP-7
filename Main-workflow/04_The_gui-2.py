...
class MLPredictionApp:
    ...
    def preprocess_features(self, df_input):
        ...
        # Standardize whitespace and casing on text columns, matching the
        # central cleaning notebook (01_cleaning). Without this, raw/dirty
        # uploads with inconsistent labels (" married", "MARRIED", "Married")
        # get one-hot-encoded into columns the model has never seen, and
        # those rows silently lose that feature's signal instead of erroring.
        X.columns = X.columns.str.strip()
        obj_cols = X.select_dtypes(include="object").columns
        for col in obj_cols:
            X[col] = X[col].astype(str).str.strip().str.title()

        # Fill missing categorical values the same way the cleaning notebook
        # does (most common value), so blank fields don't turn into their
        # own stray one-hot column or silently zero out.
        for col in obj_cols:
            if X[col].isin(["Nan", "None", ""]).any() or X[col].isna().any():
                mode_val = X[col].mode()
                if not mode_val.empty:
                    X[col] = X[col].replace(["Nan", "None", ""], mode_val.iloc[0])
