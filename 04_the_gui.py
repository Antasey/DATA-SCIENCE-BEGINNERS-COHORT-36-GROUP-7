...
class MLPredictionApp:
    ...
    def preprocess_features(self, df_input):
        ...
        # Standardize whitespace and casing on text columns
        X.columns = X.columns.str.strip()
        obj_cols = X.select_dtypes(include="object").columns
        for col in obj_cols:
            X[col] = X[col].astype(str).str.strip().str.title()

        # Fill missing categorical values safely using the mode
        for col in obj_cols:
        ...
        # Clean MonthlyIncome if formatting contains currency characters
